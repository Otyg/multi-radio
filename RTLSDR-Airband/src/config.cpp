...
static int parse_channels(libconfig::Setting& chans, device_t* dev, int i) {
    ...
    for (int j = 0; j < chans.getLength(); j++) {
        ...
        channel->emit_audio_callback = false;
        channel->emit_iq_callback = false;
#ifdef NFM
        channel->pr = 0;
        channel->pj = 0;
        ...
        if (chans[j].exists("modulation")) {
            if (strncmp(chans[j]["modulation"], "iq", 2) == 0) {
                channel_modulation = MOD_RAWIQ;
            } else
#ifdef NFM
        ...
        if (dev->mode == R_MULTICHANNEL) {
            ...
            for (int f = 0; f < channel->freq_count; f++) {
                ...
                if (chans[j].exists("modulations")) {
                    if (strncmp(chans[j]["modulations"][f], "iq", 2) == 0) {
                        channel->freqlist[f].modulation = MOD_RAWIQ;
                    } else
#ifdef NFM
...
#endif /* NFM */
        for (int f = 0; f < channel->freq_count; f++) {
            if (channel->freqlist[f].modulation == MOD_RAWIQ) {
                channel->needs_raw_iq = 1;
                break;
            }
        }
