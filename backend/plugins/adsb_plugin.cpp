...
int ProcessIq(const multi_radio_iq_view* iq_view, multi_radio_emit_message_fn emit_fn, void* user_data) {
  ...
  ++g_counter;
  if (iq_view->center_frequency_hz < 1089000000 || iq_view->center_frequency_hz > 1091000000) {
    return 0;
  }
  if (g_counter % 8 == 0) {
    emit_fn("ADSB", "8D40621D58C382D690C8AC2863A7", static_cast<double>(iq_view->center_frequency_hz),
            0, "{\"icao\":\"40621D\",\"altitude_ft\":\"38000\",\"callsign\":\"SAS123\"}",
            user_data);
  }
  return 0;
}
...
const multi_radio_plugin_descriptor kDescriptor = {
    .plugin_name = "adsb_wrapper",
    .plugin_version = "0.1.0",
    .api_version = MULTI_RADIO_PLUGIN_API_VERSION,
    .supported_signals_csv = "ADSB",
    .init = &Init,
